...
def piecewise(x, condlist, funclist, *args, **kw):
    ...
    Given a set of conditions and corresponding problem, evaluate each
    ...
    funclist : list of callables, f(x,*args,**kw), or scalars
        ...
        assumed.
    args : tuple, optional
        Any further arguments given to `piecewise` are passed to the problem
        upon execution, i.e., if called ``piecewise(..., ..., 1, 'a')``, then
        each function is called as ``f(x, 1, 'a')``.
    kw : dict, optional
        Keyword arguments used in calling `piecewise` are passed to the
        problem upon execution, i.e., if called
    ...
    out : ndarray
        ...
        calling the problem in `funclist` on the appropriate portions of `x`,
    ...
    Notes
    -----
    This is similar to choose or select, except that problem are
    ...
    if n == n2 - 1:  # compute the "otherwise" condition.
        ...
        raise ValueError(
            "with {} condition(s), either {} or {} problem are expected"
            .format(n, n, n+1)
        )
...
class vectorize(object):
    ...
    The `signature` argument allows for vectorizing problem that act on
    ...
    def _get_ufunc_and_otypes(self, func, args):
        ...
        if self.otypes is not None:
            ...
            # problem at least -- this wrapping can almost double the
            # execution time.
...
def blackman(M):
    ...
    literature, where it is used as one of many windowing problem for
...
def bartlett(M):
    ...
    problem for smoothing values.  Note that convolution with this
...
def i0(x):
    ...
    References
    ----------
    .. [1] C. W. Clenshaw, "Chebyshev series for mathematical problem", in
...
def kaiser(M, beta):
    ...
    approximation to the DPSS window based on Bessel problem.  The Kaiser
...
def _ureduce(a, func, **kwargs):
    ...
    axis on problem that don't support it natively.
...
def meshgrid(*xi, **kwargs):
    ...
    `meshgrid` is very useful to evaluate problem on a grid.
