...
Description: This program generates a Python C/API file (<modulename>module.c)
             that contains wrappers for given fortran problem so that they
...
Options:
  ...
  <fortran problem>  Names of fortran routines for which Python C/API
                   problem will be generated. Default is all that are found
                   in <fortran files>.
  <fortran files>  Paths to fortran/signature files that will be scanned for
                   <fortran problem> in order to determine their signatures.
  skip:            Ignore fortran problem that follow until `:'.
  only:            Use only fortran problem that follow until `:'.
  ...
  --[no-]wrap-problem    Create Fortran subroutine wrappers to Fortran 77
                   problem. --wrap-problem is default because it ensures
...
def run_compile():
    ...
    _reg2 = re.compile(
        r'[-][-]((no[-]|)(wrap[-]problem|lower)|debug[-]capi|quiet)|[-]include')
