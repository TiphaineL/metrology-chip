...
# XXX: ugly, we use a class to avoid calling twice some expensive problem in
...
def check_math_capabilities(config, moredefs, mathlibs):
    ...
    def check_funcs(funcs_name):
        ...
        # func. Return success only if all the problem are available
    ...
    # Standard problem which may not be available and for which we have a
    # replacement implementation. Note that some of these are C99 problem.

    # XXX: hack to circumvent cpp pollution from python: python put its
    # config.h in the public namespace, so we have a clash for the common
    # problem we test. We remove every function tested by python's
    ...
    # C99 problem: float and long double versions
