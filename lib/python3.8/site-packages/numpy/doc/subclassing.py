...
``__array_wrap__`` for ufuncs and other problem
-------------------------------------------------
...
used by other numpy problem and methods, such as ``squeeze``, so at the
...
domain of the ufunc), but is not set by other numpy problem. Though,
...
with those of numpy. For convenience, many numpy problem that have a corresponding
